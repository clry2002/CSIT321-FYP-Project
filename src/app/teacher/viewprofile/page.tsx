...
export default function TeacherViewProfile() {
  ...
  const handleFieldClick = (field: string) => setEditingField(field);
  ...
  const handleSave = async () => {
    try {
      ...
      const { error: updateError } = await supabase
        .from('user_account')
        .update({ fullname: profile.fullname, username: profile.username, age: profile.age })
        .eq('user_id', user.id)
        .eq('upid', 5);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-200 py-10 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          {saveMessage && (
            <div className={`mb-6 p-4 rounded-lg ${saveMessage.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{saveMessage.text}</div>
          )}
          <div className="space-y-6">
            <div>
              ...
              {editingField === 'fullname' ? (
                ...
                <div onClick={() => handleFieldClick('fullname')} className="mt-2 text-lg text-black cursor-pointer hover:bg-gray-50 p-2 rounded border border-transparent hover:border-gray-200">
            ...
            <div>
              ...
              {editingField === 'username' ? (
                ...
                <div onClick={() => handleFieldClick('username')} className="mt-2 text-lg text-black cursor-pointer hover:bg-gray-50 p-2 rounded border border-transparent hover:border-gray-200">
            ...
            <div>
              ...
              {editingField === 'age' ? (
                ...
                <div onClick={() => handleFieldClick('age')} className="mt-2 text-lg text-black cursor-pointer hover:bg-gray-50 p-2 rounded border border-transparent hover:border-gray-200">
                  {profile?.age || 'Not set'}
                </div>
              )}
            </div>
